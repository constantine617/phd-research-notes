...
Wang et al. (2023)研究大语言模型（Large Language Model，LLM）的推理生成：同一问题可能有不同推理路径，单次贪心 decoding 容易选中错误路径。自一致性（Self-Consistency）用多条路径的最终答案投票替代只保留一条路径。

方法先提供少样本思维链（Chain-of-Thought，CoT）prompt，再进行多次随机 sampling。每条输出被解析为一个最终答案，对相同答案汇总投票并返回票数最多者。论文也比较带生成概率权重的变体；简单多数投票是重要基线。
...
本笔记的理解是，投票比例可作为一致性信号，但它不是直接的知识证据。多个路径可能重复同一种错误，问题歧义和解析失败也会影响计票。把该比例用于不确定性量化（Uncertainty Quantification，UQ）时，需要另行验证其错误排序、校准和跨任务稳定性。
