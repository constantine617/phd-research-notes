...
Khan and Fu (2024)研究视觉问答（Visual Question Answering，VQA）中的选择性预测：当只能访问视觉语言模型（Vision-Language Model，VLM）的回答接口时，能否通过邻近问题上的一致性识别不可靠回答。
...
这篇论文把黑盒不确定性量化（Uncertainty Quantification，UQ）连接到多模态一致性和选择性问答。研究中应分别检验问题扰动质量、分数的错误排序和最终风险–覆盖率表现。
