...
正确性由任务要求和判定依据决定，不确定性由估计器及其观察条件决定。研究 UQ 的目的之一，是检验二者在何种条件下具有可用关系，而不是预先把高不确定性定义成错误。基础定义沿用[正确性与可靠性](../../hallucination/reliability-correctness.md)。

## 标签先于分数解释

对于 LLM，最终答案正确、推理过程有效、主张得到文档支持，以及格式满足要求，可以是不同标签。参考真值（Ground Truth）来自数据集、证据或人工规则，也可能存在不完整和分歧。若标签没有明确，检测器“识别错误”的说法便缺乏固定对象。

长回答尤其不能默认一个二元标签足够。[Min et al. (2023)](https://aclanthology.org/2023.emnlp-main.741/ "文献引用") 的 FActScore 拆分原子事实并判断来源支持，说明局部事实可以分别评价。主张支持比例既不等于整段完全正确概率，也不直接评价推理或任务覆盖程度。
...
[Farquhar et al. (2024)](https://www.nature.com/articles/s41586-024-07421-0 "文献引用") 将语义熵用于随生成变化出现的一类错误，并明确它难以发现持续生成同一种错误的情形。该限制来自信号所观察的变化范围，而不是调整一个阈值就能普遍解决的问题。
...
排序能力与[置信度校准（Confidence Calibration）](../../calibration/index.md)需要分别验证。[Xiong et al. (2024)](https://proceedings.iclr.cc/paper_files/paper/2024/hash/6733cf15e10e2cd1d59af033c3bb8507-Abstract-Conference.html "文献引用") 在置信度表达研究中同时考察校准和失败预测，发现改进前者并不自动解决后者。一个数值接近正确频率的估计，也仍可能无法把具体错误精确挑出。
