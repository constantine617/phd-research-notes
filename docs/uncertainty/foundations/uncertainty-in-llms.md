...
反过来，一个罕见但正确的人名、专业术语或等价改写，可能得到较低的模型概率。这种概率面向模型语言分布中的输出选择；任务正确性面向事实、推导或来源支持，两者需要通过数据建立关系。Azaria and Mitchell (2023) 在其研究中也指出，句子概率会受到长度和词频影响。
...
Geng et al. (2024) 将这些特性与语言自报置信度等能力一起讨论。LLM 的额外表达能力增加了可用信号，也增加了估计对象被混淆的机会：一段“我不确定”的文字、该段文字的概率和回答实际出错率仍需分别研究。
