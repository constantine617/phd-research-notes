...
Kadavath et al. (2022) 区分了对具体回答的自我评价与问题层面的可回答性估计。后者依赖模型将如何作答；如果更换 decoding 策略、允许检索或增加推理预算，所问的事件已经改变。研究记录应同时保存任务、回答生成规则、分数取得时点和允许使用的信息。
...
语义等价（Semantic Equivalence）使表达与含义之间出现多对一关系。Kuhn et al. (2023) 据此将概率质量聚合到含义类别。主张层面还涉及拆分规则：一个包含两个事实的句子，不能仅因句子边界清晰就视为不可再分的单一事件。
...
内部表征路线从 hidden state 提取特征，再通过探针（Probe）或距离函数构造分数。证据路线引入来源文本、检索结果或核查器输出，补充仅靠模型自身难以获得的信息。这些路线可以组合；组合是否有效，需要验证新增信号是否带来独立信息，而不能只比较更复杂的方法名称。
