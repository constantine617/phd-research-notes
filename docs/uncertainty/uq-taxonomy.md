...
LLM 的 UQ 没有唯一通用分类。一个方法可以同时使用多次生成、token 概率和语义比较；文件夹只能给它一个主要入口。**本页是知识库用于组织和比较方法的工作分类（Working Taxonomy），不是领域唯一标准。**

[Geng et al. (2024)](https://aclanthology.org/2024.naacl-long.366/ "文献引用") 按生成与分类任务进一步区分估计和校准方法；[Fadeeva et al. (2023)](https://aclanthology.org/2023.emnlp-demo.41/ "文献引用") 在 LM-Polygraph 中同时标记信息类别、访问方式和资源要求。本页吸收这些比较维度，将其整理为分别记录的分类维度。表中组合与命名属于知识库综合，不归因于某一篇论文。

## 从六个维度描述 UQ 方法

| 分类维度 | 需要记录的取值或问题 | 解释价值 |
|---|---|---|
| 信号来源 | 概率、回答分歧、语言自报、内部特征、外部核查 | 估计器究竟观察了什么 |
...
即使方法只输出一个数值，也需要按多个维度描述它。例如语义熵是一个标量，但它由含义类别上的分布得到。不能因为输出只有一个数，就把估计对象改称“回答是否正确”。类别分布还可能帮助发现问题歧义，而不仅用于风险排序。
...
## 代表性方法分别使用哪些信息、生成多少回答、输出什么结果？

| 方法或路线 | 主要信号与访问 | 生成与粒度 | 输出解释 |
|---|---|---|---|
| 长度归一化似然分数（Length-normalized Likelihood Score） | 所选 token 的 log-probability | 一条序列；序列层 | 常见程度或风险相关信号 |
...
| 正确性（Correctness）探针 | hidden state 与训练标签 | 部署时可单序列 | 对训练目标的预测分数 |
| 证据条件下的置信度 | 回答、来源及核查结果 | 主张或回答层 | 针对指定证据目标的估计 |

[Wang et al. (2023)](https://arxiv.org/abs/2203.11171v4 "文献引用") 的 Self-Consistency 原本用于推理答案聚合；本表中的频率用途是 UQ 视角的复用。[Kuhn et al. (2023)](https://arxiv.org/abs/2302.09664v3 "文献引用") 与 [Farquhar et al. (2024)](https://www.nature.com/articles/s41586-024-07421-0 "文献引用") 的语义熵需要区分理论定义、概率加权近似和频率近似。它们的具体计算统一放在[语义熵页面](sampling-based/semantic-entropy.md)。

## 分类不能替代效用比较

“单次生成”只限定目标模型生成的序列数，并未限定辅助模型调用数。一个单序列验证器可能读取很长证据并多次输出推理，实际比几条短回答更慢。“无需正确性标签”也不等于没有学习过的组件：语义比较器可能事先接受过监督训练。
...
实际检索一篇新论文时，先记录上述六个分类维度，再记录训练标签、部署条件与成本。若某项不清楚，保留未知，不凭方法名称补齐。这样可以在不复制页面的前提下发现方法之间的重叠，以及实验尚未控制的差异。
