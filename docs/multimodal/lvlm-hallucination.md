...
# 大型视觉语言模型（Large Vision-Language Model，LVLM）的幻觉

LVLM的幻觉（Hallucination）不仅涉及世界知识错误，还可能涉及对图像内容的无依据描述。文本在常识上合理，并不说明它忠于当前图像；准确复述图中文字，也不自动说明文字所述事实为真。

## 文献如何划定评价对象

Rohrbach et al. (2018) 的 CHAIR 聚焦图像描述中的对象幻觉；Li et al. (2023) 的 POPE 通过对象存在问答评价相关问题。Sun et al. (2024) 的 MMHal-Bench 涵盖对象属性、对抗性对象问题、比较、计数、空间关系、环境与整体描述等类别。以下组织借鉴这些任务维度，不主张存在唯一、互斥、穷尽的领域分类。
...
“忽略视觉证据”更接近失败机制或行为描述，不与对象和属性错误处于严格同一分类轴。一条回答可以同时符合多个维度。

## 语言先验怎样影响回答

Goyal et al. (2017) 通过同题异图异答设计检验语言先验捷径。Li et al. (2023) 观察到其研究模型容易提及常见或共现对象。Favero et al. (2024) 则比较有图与无图条件下的生成分布，研究视觉条件影响随生成过程减弱的现象。
