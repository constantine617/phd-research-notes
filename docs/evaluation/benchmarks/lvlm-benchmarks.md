...
LVLM 的评价应让图像信息真正参与任务，同时区分感知、推理、开放回答事实性与视觉定位（Visual Grounding）。一种任务得分不能覆盖全部维度。

## 互补的代表资源

| 资源 | 主要目标 | 标签与限制 |
|---|---|---|
| VQA v2（[Goyal et al., 2017](https://openaccess.thecvf.com/content_cvpr_2017/html/Goyal_Making_the_v_CVPR_2017_paper.html "文献引用")） | VQA | 同一问题配不同答案的互补图像，减弱语言先验捷径；并不消除所有偏差 |
| CHAIR（[Rohrbach et al., 2018](https://aclanthology.org/D18-1437/ "文献引用")） | 图像描述中的对象幻觉 | 根据对象标注与描述统计；不直接覆盖属性、关系或全部细节 |
| 基于轮询的物体探测评价（Polling-based Object Probing Evaluation，POPE）（[Li et al., 2023](https://aclanthology.org/2023.emnlp-main.20/ "文献引用")） | 用是否存在对象的问题检查对象幻觉 | 包括随机、常见与共现导向的负对象选择；不同负例设置难度不同 |
| MMHal-Bench（[Sun et al., 2024](https://aclanthology.org/2024.findings-acl.775/ "文献引用")） | 开放图文回答中的幻觉（Hallucination） | 涵盖属性、比较、计数、空间关系等问题；小规模挑战集及裁判依赖限制外推 |
| Flickr30k Entities（[Plummer et al., 2015](https://arxiv.org/abs/1505.04870v1 "文献引用")） | 短语与图像区域对应 | 区域定位标注；定位准确不自动说明完整生成回答正确 |
...
可用互补图像、语言单模态基线或局部证据核验检查回答是否依赖视觉。[Goyal et al. (2017)](https://openaccess.thecvf.com/content_cvpr_2017/html/Goyal_Making_the_v_CVPR_2017_paper.html "文献引用") 的设计直接针对语言先验问题；但任何单个对照都不足以证明完整因果解释。
