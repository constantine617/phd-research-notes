...
# 幻觉与事实核查 Benchmark（Hallucination Benchmarks）

幻觉（Hallucination）评价必须固定参照与粒度。本页选择互补任务，分别对应世界事实、给定来源与原子主张。不要把所有任务的失败标签统一命名后，就视为同一个参考真值（Ground Truth）。

## 任务、证据与限制

| 代表资源 | 实际评价对象 | 使用边界 |
|---|---|---|
| TruthfulQA（Lin et al., 2022） | 对常见误解相关问题生成真实且有信息量的回答 | 有意构造的难题分布；真实性与信息量应分开 |
| FEVER（Thorne et al., 2018） | 根据 Wikipedia 证据支持、反驳或无法验证主张 | 标签受证据库与检索覆盖限制；Not Enough Info 不等于世界事实为假 |
| FRANK（Pagnoni et al., 2021） | 摘要相对于来源的事实错误类别 | 针对摘要任务的分类，不能原样当作所有生成错误的唯一分类 |
| FActScore（Min et al., 2023） | 长文本原子事实的指定来源支持比例 | 是评价方法及相应研究资源；不衡量全部信息召回或任务完成度 |
| SelfCheckGPT 数据与评价（Manakul et al., 2023） | 生成式人物介绍的句子级事实性判断 | 特定生成模型、文本领域与人工标签；自一致信号不提供独立世界证据 |
