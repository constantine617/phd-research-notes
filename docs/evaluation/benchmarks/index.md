...
- [视觉语言模型](lvlm-benchmarks.md)：视觉问答（Visual Question Answering，VQA）、对象幻觉、开放多模态回答与定位。
- [医学任务](medical-benchmarks.md)：医学知识、影像问答与报告研究。

## 选择时的共同检查

[Gebru et al. (2021)](https://doi.org/10.1145/3458723 "文献引用") 强调数据组成、采集、标签、预处理与使用限制。对本研究还要记录：评价单位、正类比例、参考证据、不可判断状态、模型是否能访问同类训练材料，以及数据是否覆盖目标使用条件。
