...
AURC 汇总不同覆盖率下的选择风险，越低越好。它既受置信度（Confidence）排序影响，也受基础模型产生了多少错误影响。只比较一个面积数字，不能把两者自动分开。
...
把错误尽量放到后面，会让较低覆盖率处风险下降。同一回答池中，AURC 因而能评价拒答排序。Geifman et al. (2019) 进一步讨论减去该分类器理想排序面积的超额面积（Excess AURC，E-AURC），以减少基础错误率的直接影响。

理想排序基线依赖同一损失集合及积分约定：不能用连续近似基线减去另一种有限样本面积而不说明。E-AURC 也不应脱离基础错误率与实际覆盖率曲线单独报告。
