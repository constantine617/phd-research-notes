...
阈值指标描述一个固定决策规则的实际误报与漏检。本页把错误回答设为正类，分数不小于阈值时报警。真正例（True Positive，TP）、假正例（False Positive，FP）、真负例（True Negative，TN）与假负例（False Negative，FN）都相对于这一正类定义。
...
准确率（Accuracy）是全部判定中正确的比例；精确率（Precision）关注报警纯度；召回率（Recall）也称敏感度（Sensitivity），关注漏检；特异度（Specificity）关注负类被正确放行的比例；$F_1$ 为精确率与召回率的调和平均。这里 $TP$ 等符号均表示计数，所有比例通常在 $[0,1]$，越高越好。
...
错误稀少时，始终预测“没有错误”也能得到高准确率。Saito and Rehmsmeier (2015) 说明仅依赖这类整体数字会掩盖检出性能。报告混淆矩阵、类别比例和阈值，比只给 Accuracy 更容易判断代价。
