...
- [受试者工作特征曲线下面积（Area Under the Receiver Operating Characteristic Curve，AUROC）](auroc.md) 汇总真正率与假正率的关系，衡量正负样本之间的排序。
- [精确率–召回率曲线下面积（Area Under the Precision-Recall Curve，AUPRC）](auprc.md) 关注检出结果中有多少真正例，对正类稀少的场景尤其有帮助。
- [阈值指标](threshold-metrics.md) 描述一个实际决策阈值下的漏检与误报。

Saito and Rehmsmeier (2015) 说明，类别不平衡时，仅看受试者工作特征曲线（Receiver Operating Characteristic Curve，ROC）容易忽略大量负类带来的误报负担。因此报告错误检测结果时，应同时提供正类比例、排序指标和有实际意义的工作点。
