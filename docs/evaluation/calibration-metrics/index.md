...
校准（Calibration）评价分数所表达的概率是否与相应事件的观察频率匹配。使用前先定义事件：是预测类别正确、整段回答完全正确，还是某项主张被来源支持。概念与校准方法见[置信度校准](../../calibration/index.md)。

## 三种互补视角

- [期望校准误差](expected-calibration-error.md) 按分箱比较平均置信度与正确率，直观但依赖分箱与样本。
...
Guo et al. (2017) 讨论分箱校准误差与 NLL 等评价方式；Brier Score 的原始来源和缩放约定见其独立页面。NLL 与 Brier 都不是纯校准误差：其变化还受预测分辨能力影响。一个全体都预测总体正确率的模型可能在总体上校准，却不能识别哪条回答更容易错。
