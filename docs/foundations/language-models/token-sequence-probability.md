...
LLM 可以为下一个 token 分配概率，也可以为给定的输出序列计算概率。本页区分这些数值的定义、长度影响，以及它们与回答正确性（Correctness）之间的关系。
...
logits 可以为正或为负，也不要求相加等于一。softmax 输出的各项非负，且在整个词表上求和为一。logit 大小需要相对其他候选项解释：给所有 logits 加上同一个常数，不会改变概率分布。[Bengio et al. (2003)](https://www.jmlr.org/papers/v3/bengio03a.html "文献引用") 的神经语言模型使用 softmax 输出条件概率，其原文也讨论了数值计算中的稳定处理。
...
本页将序列结束标记视为可计分的 token。若所讨论的是生成完整回答并结束的概率，应把 序列结束标记 包含在序列中；若未包含 序列结束标记，公式描述的是相应前缀的概率。比较不同方法时，需要统一或明确这一约定。
...
这个分数通常不再是完整序列的概率，也没有在所有候选序列上归一化。不同方法可能采用不同长度处理方式；平均分数也不能保证消除所有长度相关影响。比较时还需核对 tokenizer、是否计入 序列结束标记，以及分数的正负号与方向。

## 模型概率与回答正确性

模型概率回答的是“在这个模型分布下，这段 token 序列有多大概率出现”。回答正确性则需要依据任务、事实或给定证据判断。二者对应不同事件。

因此，即使某段文本获得较高模型概率，也不能直接把该数值当作回答正确的概率。这首先是概率对象的区别，并不只是数值是否经过校准的问题。

置信度（Confidence）若用于表示回答正确的可能性，还需要检查置信度校准。[Guo et al. (2017)](https://proceedings.mlr.press/v70/guo17a.html "文献引用") 在分类问题中研究了置信度与实际正确率的匹配。这里借用该校准目标，应用到回答层面时仍须先明确正确性事件与判断规则。

## 与当前研究的关系

UQ 可以从这些概率构造信号，但用于幻觉或错误检测时，需要验证信号与目标错误的关系。实验中应说明使用的是单 token 概率、序列 log probability，还是某种归一化分数。
