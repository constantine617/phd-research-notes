...
在离散分类中，假设已观察到类别 $y$，并用 one-hot 目标 $p_y$ 表示：$p_y(y)=1$，其他类别概率为零。式 $\eqref{eq:cross-entropy-definition}$ 简化为：
...
多个位置的损失还需要一个聚合规则。若两个位置的目标概率分别为 $0.8$ 和 $0.2$，其交叉熵之和约为 $1.833$，平均约为 $0.916$。这两个数来自同一预测，区别只是分母；不报告聚合方式，就不能进行有意义的数值比较。
