...
## 概率（Probability）：固定模型，考察结果

设 $p(y\mid x,\theta)$ 是一个条件概率（Conditional Probability）模型。其中，$x$ 是输入或上下文，$y$ 是待预测结果，$\theta$ 是模型参数。固定 $x$ 和 $\theta$ 后，让 $y$ 遍历可能结果，就得到一个分布。对互斥且穷尽的离散结果，其概率非负，合计为 $1$。
...
在语言模型中，条件通常写成具体上下文，而不是展开为事件集合。给定“今天的天气很”与给定“这道题的答案是”，下一个 token 面临的条件不同，即使 vocabulary 相同也不应期待分布相同。条件概率与概率链式法则的基础说明可参见 [Goodfellow et al. (2016)](https://www.deeplearningbook.org/ "文献引用")。

## 似然（Likelihood）：固定数据，考察参数

设已经观察到数据 $D$。似然将这些数据固定，把概率模型看成参数 $\theta$ 的函数：
...
LLM 训练时，可以固定已观察到的 token 序列，通过调整参数提高其条件似然；通常的 inference 则固定参数，预测或生成新的 token。神经语言建模中的序列概率与似然训练可参见 [Bengio et al. (2003)](https://www.jmlr.org/papers/v3/bengio03a.html "文献引用")。

对 UQ 而言，模型概率是可用信号，但“模型会生成这个回答”与“这个回答正确”是不同事件。要把分数解释为正确率，需要另行定义正确性目标并验证校准关系。
