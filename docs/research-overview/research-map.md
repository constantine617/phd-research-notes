...
下面两张图分别展示整体研究范围和当前 UQ 方向。整体范围是 LLM 与 LVLM 的可信性；UQ 是当前主要技术线。图中的关系用于组织研究，不表示方法已完成，也不构成固定时间安排。
...
```mermaid
flowchart LR
    T["可信 LLM / LVLM"] --> G["事实性、忠实性<br/>与鲁棒性"]
    T --> H["幻觉评估与缓解"]
    T --> R["输出可靠性估计"]
    T --> M["多模态可信性"]
...
两张图的实线表示研究范围中的包含或展开关系。图中只保留理解研究结构所需的关系，方向之间的技术联系在下文说明。

整体范围由可信性目标约束。幻觉评估与缓解、输出可靠性估计，以及多模态可信性分别保留位置，便于长期研究围绕不同问题展开。多模态可信性包括图文表征与对齐、模型训练和医疗 LVLM 应用；其中，图文表征与对齐、模型训练也可以形成独立的技术路线。长期边界见[研究范围（Research Scope）](research-scope.md)。
...
未来向多模态扩展时，可以研究外部证据（External Evidence）和多模态证据（Multimodal Evidence）如何与不确定性信号结合。这是证据感知 UQ 与多模态可信性之间的一条技术联系；多模态可信性仍直接属于整体研究范围，保留其长期研究地位。
